...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Texts for Discussion](https://danilofreire.github.io/datasci385/lectures/lecture-23/23-discussion.html)"
transition: slide
...
- The first uses a [list experiment]{.alert} to measure underreporting of drug and alcohol use amongst athletes (a major problem in athletics!)
- The second paper is a more sophisticated one: the authors use [two methods]{.alert} to measure support for NATO forces in Afghanistan (a sensitive topic indeed!)
- The third paper uses a clever design, leveraging a referendum in the US to see which method is better to measure sensitive topics (spoiler: [randomised response]{.alert} is the best one!)
- The last one, by yours truly, uses a [conjoint experiment]{.alert} to address a major problem in developing countries: support for extralegal violence
- And I can say that [the issue seems not to be that sensitive to a lot of respondents]{.alert} 😅
- I hope you find them useful and interesting! 😊
:::
:::
:::

# Drug and alcohol use amongst athletes 📋 {background-color="#1B3A6B"}
...
- However, [there is widespread concern that athletes underreport their drug and alcohol use in surveys]{.alert}
- "[...] _among male student-athletes, 9.7 percent say they use “banned performance enhancers” and 55.8 percent say they used “performance-enhancing drugs” (which might include legal nutritional supplements). Among female student-athletes, no one said they use “banned performance enhancers” and 29.8 percent said they used “performance-enhancing drugs.”_" (p. 370)
- In an official NCAA survey, [only 0.4 per cent of athletes self-reported using anabolic steroids]{.alert}
- The authors argue that [list experiments can help uncover the true prevalence of drug and alcohol use]{.alert} (or at least get closer to it)
...
- They accessed the athletic websites of all 12 Big Ten universities (yes, the "Big Ten" includes 12 universities!) and obtained [the full rosters for all sports at every school]{.alert}
...
> We are interested in HOW MANY of these activities you have engaged in – NOT which ones. 
...
- [Direct question]{.alert} =  4.9 per cent 😂
...
- Again, [large statistical difference]{.alert}:
  ...
  - [46 per cent consumed more than five drinks a week]{.alert}
- Less than [3 per cent]{.alert} of athletes reported that they consumed more than five drinks a week when asked directly
- The difference is [even larger than for drug use]{.alert}
- [What do you think about this result?]{.alert}
:::

# Combining list and endorsement experiments {background-color="#1B3A6B"}
...
- Here, for the sake of time, we will focus on [their experimental design]{.alert}, but I encourage you to read the paper and see how the authors developed their estimators! 🤓
...
:::{.column width="50%"}
:::{style="text-align: center; margin-top: -30px;"}
![](figures/blair-figure01a.png){width=70%}
...
:::{.column width="50%"}
- [Both methods yield similar results!]{.alert}
...
- The result indicates that the efforts to win "hearts and minds" had [limited success]{.alert}
:::
:::
:::
...
- [ISAF victimisation is associated with a consistently negative effect across all three models]{.alert}
...
- [The authors don't know exactly why this happens]{.alert} 
- ISAF post-harm efforts appear to have a [positive effect on ISAF support]{.alert}
...
:::{style="font-size: 26px; margin-top: 30px; text-align: center;"}
![](figures/blair-figure04.png){width=80%}
...
:::{style="font-size: 26px; margin-top: 30px; text-align: center;"}
![](figures/blair-figure03.png){width=70%}

Source: [Blair et al. (2014)](https://onlinelibrary.wiley.com/doi/abs/10.1111/ajps.12086)
:::

# Empirical Validation of Survey Methodologies for Sensitive Questions {background-color="#1B3A6B"}
...
:::{.columns}
:::{.column width="50%"}
- In 2011, Mississippi voters were asked to vote on a [proposition to define life as beginning at conception]{.alert} 
- Most commentators expected the initiative to pass easily, but the opposite happened
- The amendment was defeated 57.6% to 42.4%, a [15 percentage point swing from the pre-election poll]{.alert}
...
- They sampled [only from counties that had an unexpected number of "no" votes]{.alert}
:::
...
> * [Voted ‘YES’ on the ‘Personhood’ Initiative on the November 2011]{.alert}
:::
:::
:::
...
>
> Phil Bryant, Governor of Mississippi, [who campaigned in favor of the ‘Personhood’ Initiative on the 2011 Mississippi General Election
ballot?]{.alert}
>
>Very favorable
>
...
- The rest were maybe [confused by the procedure]{.alert}, which is indeed one of the [disadvantages of this method]{.alert}
:::
...
:::{style="font-size: 26px; margin-top: 30px; text-align: center;"}
![](figures/rosenfeld-figure05.png){width=60%}

Source: [Rosenfeld et al. (2016)](https://onlinelibrary.wiley.com/doi/abs/10.1111/ajps.12205)
:::

# Last one! <br> Support for vigilantism in Brazil 🇧🇷 {background-color="#1B3A6B"}
...
:::{.columns}
:::{.column width="50%"}
- We ran the survey online with [Qualtrics]{.alert}
...
# Thank you for your attention! 🙏 {background-color="#1B3A6B"}