...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Attrition](https://danilofreire.github.io/datasci385/lectures/lecture-11/11-attrition.html)"
transition: slide
...
# Attrition in Experiments 🧩 {background-color="#1B3A6B"}
...
# Some notation 📝 {background-color="#1B3A6B"}
...
- We can classify individuals into four types based on their response behaviour [(Lee 2009)](https://academic.oup.com/restud/article/76/3/1071/1590707):
  ...
  - [Always-reporters:]{.alert} $r_i(0) = 1$ and $r_i(1) = 1$ — they always report their outcomes regardless of treatment assignment
  - [Never-reporters:]{.alert} $r_i(0) = 0$ and $r_i(1) = 0$ — regardless of treatment status, they always attrite on the outcome
  - [If-treated reporters:]{.alert} $r_i(0) = 0$ and $r_i(1) = 1$ — they report their outcomes only if assigned to treatment
  - [If-untreated reporters:]{.alert} $r_i(0) = 1$ and $r_i(1) = 0$ — they report their outcomes only if assigned to control
...
# Special cases of attrition {background-color="#1B3A6B"}
...
# Inverse probability weighting {background-color="#1B3A6B"}
...
# ATE Bounds {background-color="#1B3A6B"}
...
# Handling attrition in R {background-color="#1B3A6B"}
...
#| message: false
#| warning: false
# attritevis calls Hmisc::Lag() internally without importing it; load Hmisc so the package works
