...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Quasi-Experiments](https://danilofreire.github.io/datasci385/lectures/lecture-15/15-quasi.html)"
transition: slide
...
- [Key historical roots]{.alert}: FDA Modernization Act of 1997, reproducibility crisis
- [Important elements]{.alert}:
...
- Some scholars say [PAPs are overrated]{.alert}, but they do help with credibility
- Best practices for writing PAPs:
...
- [Quasi-experiments]{.alert}: Broader term than natural experiments, sometimes we can control the treatment but not the assignment
  - [Examples]{.alert}: School vouchers, minimum wage laws, health interventions, policy changes
  - [Methods to estimate causal effects]{.alert}: Difference-in-differences, regression discontinuity, instrumental variables
- They are [somewhere between observational and experimental studies]{.alert}, and have received increased attention in recent decades
:::
...
# The experimental ideal and its limits 🌟 {background-color="#1B3A6B"}
...
- But they are often the best available or only feasible options
...
# Natural experiments 🌍 {background-color="#1B3A6B"}
...
- [The challenge]{.alert}: Students who choose to attend charter schools are likely to be different from those who don't
    -  Maybe more motivated families, different prior academic preparation, etc.
    -  This is [selection bias!]{.alert}
-  We need a way to isolate the causal effect of charter schools, not just correlation
:::

:::{.column width="50%"}
- [The clever idea]{.alert}:  Many charter schools are oversubscribed and use lotteries to decide who gets in
- [Instrumental Variable (IV) Approach]{.alert}:  Use a lottery win as an instrument to estimate the causal effect of charter attendance.
...
-  Lots of control variables (year ($\alpha_{2t}$) and grade ($\beta_{2g}$) fixed effects, dummies for combinations of charter school lotteries ($d_{ij}$), demographic characteristics ($X_i$), etc.) to account for other factors
:::

:::{.column width="50%"}
- [Equation (2)]{.alert}: First Stage (How the lottery affects attendance)
...
# Quasi-experiments 📊 {background-color="#1B3A6B"}
...
- Examples of Quasi-Experimental Designs (we also use them to analyse natural experiments!):
...
:::{style="margin-top: 30px; font-size: 26px;"}
- RDD exploits sharp or fuzzy discontinuities in treatment assignment based on a [threshold or cutoff score]{.alert}
...
- [Local Average Treatment Effect (LATE)]{.alert}: RDD estimates the effect for those near the cutoff
...
Source: [Mignozzetti, U., et al. (2024). Legislature size and welfare: Evidence from Brazil, American Journal of Political Science](https://doi.org/10.1111/ajps.12843)
:::
...
Source: [Card, D., & Krueger, A. B. (1994). Minimum Wages and Employment: A Case Study of the Fast-Food Industry in New Jersey and Pennsylvania. The American Economic Review, 84(4), 772-793.](https://www.mattblackwell.org/files/teaching/CarKru94.pdf)
:::
...
# Threats to validity and mitigation strategies {background-color="#1B3A6B"}
