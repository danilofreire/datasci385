...
subtitle: Lecture 08 - Blocking, Clustering, and Statistical Power
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Power](https://danilofreire.github.io/datasci385/lectures/lecture-08/08-power.html)"
transition: slide
...
# Hi, there! <br> Nice to see you again! 😉{background-color="#1B3A6B"}
...
- Common in experiments where [individual randomisation is impossible]{.alert} (e.g., classrooms, villages)
...
# Blocking (cont.) {background-color="#1B3A6B"}
...
- Simulating our experiment using `DeclareDesign`. First, we use [declare_model()](https://declaredesign.org/r/declaredesign/reference/declare_model.html) to specify our data-generating process
...
- Next, we use [declare_inquiry()](https://declaredesign.org/r/declaredesign/reference/declare_inquiry.html) to specify our estimand of interest
```{r}
#| echo: true
#| eval: true
...
:::{style="margin-top: 30px; font-size: 26px;"}
- Now, we use [declare_assignment()](https://declaredesign.org/r/declaredesign/reference/declare_assignment.html) to specify our random assignment mechanism
```{r}
#| echo: true
#| eval: true
...
- Here we use [declare_measurement()](https://declaredesign.org/r/declaredesign/reference/declare_measurement.html) to specify how we measure our outcomes
```{r}
#| echo: true
#| eval: true
...
- Finally, we use [declare_estimator()](https://declaredesign.org/r/declaredesign/reference/declare_estimator.html) to specify our estimation strategy (how we estimate the treatment effect)
```{r}
#| echo: true
#| eval: true
...
- We just reuse the same model and inquiry for a fair comparison, but now we do not block in the assignment step
```{r}
#| echo: true
#| eval: true
...
- `simulate_design()` allows us to simulate multiple designs and compare their performance
- We will simulate both designs (with and without blocking) 1000 times each and compare the distribution of estimated treatment effects
```{r}
#| echo: true
#| eval: true
...
# Cool, isn't it? 🤓 {background-color="#1B3A6B"}

# Clustering (cont.) {background-color="#1B3A6B"}
...
- The measure ranges from 0 to 1. When it is closer to 0, it means that clusters have [no influence]{.alert} on the outcome, so we can treat individuals as independent
...
:::
- Numerator ($n$): Sample size
- Denominator: [$1 + (k - 1) * ICC$]. The [variance inflation factor]{.alert}: This is the key part that adjusts for clustering
- Let's look at it piece by piece:
  ...
  - $(k - 1) \cdot ICC$: Total amount of correlation effect within a cluster. ICC multiplied by $k-1$ because each individual is correlated with $k-1$ other individuals in their cluster 
  - Dividing by the denominator: By dividing the nominal sample size $n$ by this variance inflation factor, we indicate that our clustered sample provides less independent information. If there were no clustering effect ($ICC = 0$), the denominator would be 1, and ESS would equal $n$
:::
:::
:::
...
cluster_design <- 
  ...
  declare_model(clusters = add_level(N = N_clusters, 
                                          cl_size = rep(c(100, 10), c(N_clusters/6, N_clusters - N_clusters/6)),
...
# Statistical power 💪 {background-color="#1B3A6B"}
...
- The first controls for the pre-treatment covariate $W$ in OLS  
...
# Final thoughts 💭 {background-color="#1B3A6B"}
