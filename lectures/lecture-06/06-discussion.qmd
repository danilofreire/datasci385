...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Discussion](https://danilofreire.github.io/datasci385/lectures/lecture-06/06-discussion.html)"
transition: slide
...
# Hi, there! 👋 <br> Tudo bem? 😄🇧🇷 {background-color="#1B3A6B"}
...
- Compared the [Neyman-Rubin model]{.alert} (ATE) with the [Ronald Fisher model]{.alert} (sharp null hypothesis)
- Talked about [Type I and Type II errors]{.alert} 
- Discussed [sampling variability]{.alert} and how it affects the interpretation of experimental estimates
- Introduced [randomisation inference]{.alert} with the `ri2` package
...
- I'd like you to start working on your pre-analysis plan as soon as possible 🤓
...
# R packages 📦 {background-color="#1B3A6B"}
...
- They even have a tutorial showing how to [create common social science data structures](https://declaredesign.org/r/fabricatr/articles/common_social.html) (and more!)
- So I recommend that you check it out and play around with it 😉
...
- It also has all the types of randomisation we use in experiments
...
participants$assignment <- block_ra(
  blocks = participants$gender
)
table(participants$assignment, participants$gender)
```  
```{r}
#| echo: TRUE
#| eval: TRUE
...
students <- data.frame(
  student_id = 1:2000,
  school_id = rep(1:10, each = 200) 
  # Each school (10) is a cluster of 200 students
)

# `m = 5` to assign 5 clusters to the treatment condition
students$assignment <- cluster_ra(
  clusters = students$school_id, m = 5
)
table(students$assignment)
```
:::
:::
:::
...
- An easy way to estimate sharp null hypothesis tests
...
# Papers for Discussion 📚 {background-color="#1B3A6B"}
...
:::{.column width="50%"}
- [Overview]{.alert}
  - Do campaign donations secure preferential treatment from policymakers?
  ...
  - Do donations *facilitate access* to influential policymakers?
...
- [Intervention]{.alert}
  ...
  - Control Condition: Offices are informed that the meeting attendees are “local constituents”
...
- [Outcome Measurement]{.alert}
  ...
  - Data Collection:  The level of staff who agreed to the meeting was recorded
...
- [Random Assignment]{.alert}
  ...
  - Within the blocks, offices were randomly assigned to the 'donor revealed' condition
...
# Bertrand and Mullainathan (2004) 🇺🇸 {background-color="#1B3A6B"}
...
# Chattopadhyay and Duflo (2004) 🇮🇳 {background-color="#1B3A6B"}
...
- [The Experiment]{.alert}
  ...
  - Key Finding: Reservation affects the types of public goods provided, with leaders investing more in infrastructure relevant to their own gender
:::
...
- [Intervention]{.alert}
  ...
  - Treatment: Whether the village council was randomly reserved for a woman leader
...
- [The Process]{.alert}
  ...
  - Surveys collected in the summer of 2000 (West Bengal and Rajasthan) and at the end of 2002 (Rajasthan)
:::
:::
:::
...
:::{style="margin-top: 30px; font-size: 21px; text-align: center;"}
![](figures/results-duflo3.png){width=80%}
:::
...
- [Implications]{.alert}
 ...
 - [Theoretical Contribution]{.alert}
    - Evidence of the effect of identity on the policymaking process
    - Contradicts Downsian models of policy preferences (which assume that politicians are purely self-interested)
    - Shows the importance of the design of political institutions in ensuring the representation of different social groups
- [Limitations]{.alert}
  ...
  - Mechanisms: the exact mechanisms of the decision-making process are still not fully clear
:::

# Questions? 😉 {background-color="#1B3A6B"}

# And that's it for today! 🎉 {background-color="#1B3A6B"}

# All the very best! <br> See you soon! 🚀 {background-color="#1B3A6B"}