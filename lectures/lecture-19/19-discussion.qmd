...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Texts for Discussion](https://danilofreire.github.io/datasci385/lectures/lecture-19/19-discussion.html)"
transition: slide
...
- Just a quick reminder that I am teaching [DATASCI 185 - Introduction to AI Applications]{.alert} next semester 🤖
...
# Today's plan 📅 {background-color="#1B3A6B"}
...
# Ready for some applied causal inference? 🤓 {background-color="#1B3A6B"}

# But first, the experiment of the week 🔬 {background-color="#1B3A6B"}
...
# Part 1: Munshi (2003) - Networks in the Modern Economy 🌐 {background-color="#1B3A6B"}

## Introduction: The Power of Networks

:::{style="margin-top: 30px; font-size: 28px;"}
- [Do social networks improve labour market outcomes for Mexican migrants in the U.S.?]{.alert}
- [Motivation:]{.alert}
  - Non-market institutions (like networks) are really important to the economy, especially where markets are imperfect (e.g., [information gaps in labour markets]{.alert}).
  - Migrant communities often rely heavily on [social ties]{.alert} for finding jobs, housing, etc. (Rees 1966, Montgomery 1991)
  - Particularly relevant for [undocumented migrants]{.alert} who face barriers to formal channels
- [Does a larger network (from the same origin community) lead to better employment prospects and higher-paying jobs?]{.alert}
:::
...
- Who contributes? [Established migrants]{.alert} (longer duration, more connections, more employed)
- Who benefits? [New arrivals]{.alert} or those who would otherwise struggle to find employment/good jobs
- Networks could lead to [higher employment rates]{.alert} and channel members into [better (e.g., non-agricultural) jobs]{.alert}
:::
...
- [Network Definition:]{.alert} For an individual $i$ from community $c$ in year $t$, the network size ($X_{t-1}$) is measured as the [proportion of sampled individuals from community $c$ who were located in the U.S. in the *previous* year ($t-1$)]{.alert}
...
- The analysis later refines this by distinguishing between [new migrants]{.alert} (in the U.S. < 3 years) and [established migrants]{.alert} (in the U.S. ≥ 3 years), based on theoretical predictions
:::

## Identification Challenge 1: Selection Bias

:::{style="margin-top: 30px; font-size: 28px;"}
- Migrants are [not a random sample of the origin community]{.alert}
...
- The author uses panel data (repeated observations for the same individual over time) and include [individual fixed effects]{.alert} ($\alpha_i$)
- Fixed effects control for any [time-invariant unobserved characteristics]{.alert} of the individual (like innate ability, stable motivation)
- The effect of the network ($\beta$) is now identified from [changes in network size *over time* for the *same individual*]{.alert}, comparing their outcomes when their network was larger versus smaller
- [Important assumption:]{.alert} Unobserved individual factors ($\alpha_i$) are constant over the sample period (perhaps reasonable for low-skill jobs)
...
:::{style="margin-top: 30px; font-size: 28px;"}
- Thus, the author needs to find an instrument ($Z$) that satisfies two conditions:
...
- [Checking Relevance:]{.alert} Is Mexican rainfall correlated with network size in the U.S.?
...
- [Checking the Exclusion Restriction:]{.alert} Does Mexican rainfall affect U.S. employment *only* via network size?
    - This seems plausible too: Origin communities are far from U.S. destinations. Mexican weather shocks should be uncorrelated with U.S. labour market shocks. (Munshi reports a [correlation coefficient of 0.01]{.alert} between origin rainfall and U.S. destination rainfall)
    - Could low rainfall affect migrant characteristics (e.g., desperation, reservation wage) directly influencing U.S. employment? Munshi argues the observed [timing (long lags)]{.alert} makes this less likely to be the primary channel for the *employment* effect
:::
...
:::{.columns}
:::{.column width="50%"}
- **Table IV** directly regresses employment (in the U.S. or Mexico) on lagged rainfall, controlling for individual FE and year FE
- For **U.S. Employment (Column 1):**
    - [Recent-past rainfall]{.alert} ($t$ to $t-3$) shows *no significant effect*
    - [Distant-past rainfall]{.alert} ($t-4$ to $t-6$) shows a [negative and significant effect]{.alert}. Less rain many years ago correlates with higher current U.S. employment
...
- It plots employment (U.S.) and network size (established migrants) against distant-past rainfall, after netting out fixed effects
- The figure visually suggests:
    - As distant-past rainfall increases (wetter conditions in Mexico years ago), current U.S. employment tends to [decrease]{.alert}
...
:::{.columns}
:::{.column width="50%"}
- **Table VI** compares OLS and IV estimates of network effects on U.S. employment probability
...
- Further robustness checks (columns 3-11) support these patterns. Effects seem larger for women/older men/less educated, and when restricting to fresh arrivals
:::
...
- This study finds that
    - [Social networks significantly improve labour market outcomes]{.alert} for Mexican migrants in the U.S., increasing both employment probability and access to better (non-agricultural) jobs
...
:::{style="margin-top: 30px; font-size: 28px;"}
1.  How convincing is the rainfall instrument's [exclusion restriction]{.alert}? Could distant rainfall affect current U.S. employment through channels *other* than the measured network size (e.g., affecting savings, remittances, characteristics of the migrant pool)?
...
3.  What are the limitations of measuring the network as the [proportion of the sampled community]{.alert} in the U.S.? How might alternative measures change the story?
...
- Mellon is not very convinced that rainfall is a good instrument 😂
- What do you think?
:::
...
# Part 2: Miguel & Kramer (2004) - Worms 🐛 {background-color="#1B3A6B"}
...
:::{.columns}
:::{.column width="50%"}
- [What is the impact of school-based mass deworming programmes on children's health and education (school participation, test scores)?]{.alert}
- Intestinal worms (helminths) are extremely common in developing countries, especially among school-aged children. Associated with malnutrition, listlessness
- [Key Problem:]{.alert} Previous studies often randomised treatment *at the individual level*. This likely *underestimates* the true benefits due to [treatment externalities (spillovers)]{.alert}
:::
...
- [Externality:]{.alert} Treating child A can benefit untreated child B if they live/play/attend school nearby, because B is less likely to get infected or re-infected
- This creates issues for individual-level randomisation:
    ...
    - This represents a violation of the [Stable Unit Treatment Value Assumption (SUTVA)]{.alert} (but a good one!)
:::
...
- Advantages of this design:
    ...
    2.  Random assignment at the [*school level*]{.alert} generates variation in the *local density* of treatment, enabling estimation of [cross-school externalities]{.alert}
    3.  More ethical than a permanent control group, as all schools eventually receive treatment
:::
:::
:::
...
:::{style="margin-top: 30px; font-size: 26px;"}
- [Overall Effect (ITT at School Level):]{.alert} Compare mean outcomes in treatment schools vs. comparison schools in a given year, controlling for baseline differences
...
- [Cross-School Externalities:]{.alert} Exploit the [exogenous variation]{.alert} in treatment density induced by school-level randomisation
...
- [Within-School Externalities:]{.alert} Harder to identify experimentally due to lack of within-school randomisation. The paper uses non-experimental comparisons (e.g., untreated pupils in T schools vs C schools - Table VI), acknowledging potential selection issues
:::
...
- **Comparison Schools:** Very low rates (<3%) of pupils receiving deworming drugs independently
:::
...
- **Health Perspective:** Cost per infection averted is estimated around [$5]{.alert} (including externalities), making it highly cost-effective by WHO standards, especially driven by schistosomiasis impact. Geohelminth treatment alone appears less cost-effective based *only* on health metrics
...
- Methodologically, it nicely illustrates the importance of accounting for externalities in program evaluation. It shows how [school-level (or cluster) randomisation]{.alert} can facilitate the identification of overall effects and the estimation of spillovers
...
- [Miguel & Kramer (2004) Takeaways:]{.alert}
...
# Thanks very much! 😊 {background-color="#1B3A6B"}

# See you next time! 👋 {background-color="#1B3A6B"}