...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Non-Compliance 2](https://danilofreire.github.io/datasci385/lectures/lecture-10/10-non-compliance2.html)"
transition: slide
...
# Hi, there! <br> Hope all is well! 😉 {background-color="#1B3A6B"}
...
# Two-sided non-compliance 🤔 {background-color="#1B3A6B"}
...
# Compliance types 📊 {background-color="#1B3A6B"}
...
- The term is 1 only for those who follow their assignment—taking the treatment if offered ($d_i(1)=1$) and not otherwise ($d_i(0)=0$).
...
# Monotonicity 📏 {background-color="#1B3A6B"}
...
- But if we assume that's not the case (no defiers), all problems are solved! 😂
...
# Now the easy part, CACE estimation 🤓 {background-color="#1B3A6B"}
...
# Back to our example 📊 {background-color="#1B3A6B"}
...
- [Wait a minute!]{.alert} Where do these numbers come from?
  ...
  - $\frac{185}{505} \times 100 \approx 36.6$% were treated in the treatment group [Aha!]{.alert} 😂
...
# Now you're experts in non-compliance 😎 {background-color="#1B3A6B"}
...
# How would you improve the experiment? 🤓 {background-color="#1B3A6B"}
...
- Please also send me your ideas for the [group project]{.alert}! I'm looking forward to hearing from you! 🤓
