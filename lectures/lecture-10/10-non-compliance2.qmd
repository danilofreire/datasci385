...
- With $\pi_{D} = 0$, we can estimate all proportions using observed data:
    - [Always-Takers]{.alert} ($\hat{\pi}_{AT}$), people who watch the debate regardless of assignment:
    - $\hat{\pi}_{AT} + \hat{\pi}_{D} = 0.16 \implies \hat{\pi}_{AT} = 0.16$
    - [Never-Takers]{.alert} ($\hat{\pi}_{NT}$), those who never watch the debate:
...
- Although two-sided noncompliance introduces the possibility that some subjects are Always-Takers, they pose [no identification problems]{.alert}
- As the share of Always-Takers is the same in both treatment and control groups, they [do not affect the difference in outcomes between groups]{.alert}
:::
