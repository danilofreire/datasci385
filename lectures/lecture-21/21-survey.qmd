...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Survey Experiments](https://danilofreire.github.io/datasci385/lectures/lecture-21/21-survey.html)"
transition: slide
...
# Hey, there! 🤓 {background-color="#1B3A6B"}
...
# Activity! 😃 {background-color="#1B3A6B"}
...
:::{style="font-size:35px; margin-top: 34px;"}
...
:::{.columns}
:::{.column width="50%"}
- [The population of Madrid is approximately 3.3 million people]{.alert}
- What did you guess?
- What’s going on here?  
  - A survey experiment!  
  - Demonstrates [“anchoring” heuristic]{.alert}
- Survey experiments are easy to analyse, but [only if designed and implemented well!]{.alert}
:::
...
# Survey experiments 📊 {background-color="#1B3A6B"}
...
# What makes a good survey experiment? 🧐 {background-color="#1B3A6B"}
...
:::{.columns}
:::{.column width="50%"}
- [Manipulation checks]{.alert} are items [added post-treatment and/or post-outcome]{.alert} that assess whether the [independent variable was affected by treatment]{.alert}
...
- Measures an outcome that [should not be affected by the independent variable]{.alert} 
- Example: 
...
# Common paradigms and examples {background-color="#1B3A6B"}
...
- Example: 
  ...
  - If answers to manipulated questions matter, we can measure the rest post-outcome
:::
...
# Concluding remarks 📝 {background-color="#1B3A6B"}
