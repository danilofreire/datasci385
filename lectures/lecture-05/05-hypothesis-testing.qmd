...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Hypothesis Testing](https://danilofreire.github.io/datasci385/lectures/lecture-05/05-hypothesis-testing.html)"
transition: slide
...
# Hello, everyone! 👋 <br> How are you doing today? 😁 {background-color="#1B3A6B"}
...
- Regression analysis is the most convenient way to estimate treatment effects
...
- It will be a group project, with 4-5 students per group
...
# Regression analysis in R (recap) 📊 {background-color="#1B3A6B"}
...
# Hypothesis Testing 👩🏻‍🔬 {background-color="#1B3A6B"}
...
:::{.column width=50%}
:::{style="text-align: center; margin-top: -40px; font-size: 19px;"}
...
- Higher [statistical power]{.alert} (1 - $\beta$) means better detection of real effects
- Researchers aim for high power (often 80% or more), typically by increasing the sample size, to reduce the chance of missing a genuine discovery
- We will have a whole session on statistical power later in the course 😉
:::
:::
:::
:::
...
- Neyman had such a huge impact on experimental methods that many scholars call the potential outcomes framework [the Neyman-Rubin model]{.alert}
...
- Let's see how it works!
- Another package that I find useful (and broadly similar to `ri2`) is the `ritest` package, available on GitHub: <https://github.com/grantmcdermott/ritest>
:::
...
## Another example!
### Effect of not having a runoff in sub-Saharan Africa

:::{style="margin-top: 50px; font-size: 21px;"}
:::{.columns}
:::{.column width=45%}
- The data to the right, from [Glynn and Ichino (2015)](https://onlinelibrary.wiley.com/doi/abs/10.1111/ajps.12154), examine the relationship between the absence of a runoff election ($A_i = 1$) and harassment of opposition parties ($Y_i$)
 - Data were collected from 10 sub-Saharan African countries
...
# Let's discuss a real experiment! 🤓 {background-color="#1B3A6B"}
...
### Based on the [EGAP Research Design Form](https://danilofreire.github.io/datasci385/design-form.html)
...
-   [Control]{.alert}:
    ...
    -   Pure control (no intervention); controls for external factors and placebo effects associated with knowing about the existence of the app
:::
...
-   [Measurement]{.alert}:
    ...
    -   Data come from the Ministry of Education's SIMEC platform and were accessed via its API
...
-   [Covariates]{.alert}:
  ...
  -   No additional outcomes or covariates collected to distinguish between explanations and alternatives; however, the randomisation at the municipality/school level is designed to account for these biases
-   [Ethics]{.alert}:
...
-   [Data management]{.alert}:
  -   Data were stored securely on cloud servers
  -   Data were anonymised by using city/school identification and not user information
  -   Publicly available data were collected directly via APIs or downloaded directly from the ministry's site
:::
...
# ...and that's it! 😊 {background-color="#1B3A6B"}
