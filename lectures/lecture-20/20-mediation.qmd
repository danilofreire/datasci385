...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Mediation](https://danilofreire.github.io/datasci385/lectures/lecture-20/20-mediation.html)"
transition: slide
...
- [**Paper 1: Munshi (2003)**](https://doi.org/10.1162/003355303321675455)
  - [Goal]{.alert}: Identifying network effects in the labour market
  - [Context]{.alert}: Mexican migrants in the US
  - [Challenge]{.alert}: Endogeneity and selection bias
  - [Method]{.alert}: Instrumental Variables (IV) using rainfall
:::

::: {.column width="50%"}
- [**Paper 2: Miguel & Kremer (2004)**](https://onlinelibrary.wiley.com/doi/abs/10.1111/j.1468-0262.2004.00481.x)
  - [Goal]{.alert}: Identifying treatment impacts with externalities
...
- The focus of last class was on [clever identification strategies]{.alert} when standard RCT assumptions are violated or RCTs are not feasible 
:::
:::
:::
...
- Can we [experimentally manipulate mediators]{.alert}? 
  - Factorial designs
...
# What is mediation? 🤔 {background-color="#1B3A6B"}
...
- Goal: [Identify the pathways]{.alert} through which $Z$ influences $Y$
:::
...
- [Designing Better Interventions]{.alert}: If we know *why* something works, we might find more efficient or potent ways to achieve the same outcome (e.g., Vitamin C tablets instead of limes)
  - [Mediators themselves can have mediators!]{.alert} Yes, really! Can you give an example?
...
# The traditional approach: regression 📈 {background-color="#1B3A6B"}
...
- $Z$ is randomly assigned (uncorrelated with errors, unbiased estimates of $Z$ in equations 01 and 02), but $M$ [is *not*]{.alert} (equation 03). $M$ is an outcome of $Z$ (neither randomised nor pre-treatment). [So equation 03 is an observational study]{.alert} (and it can be biased)!
:::
...
:::{.columns}
:::{.column width=50%}
- If we assume [constant treatment effects]{.alert} (a, b, c, d are the same for everyone) and assume no unobserved confounding between $M$ and $Y$)...
- The total effect (c) can be decomposed:
  - [Total Effect (c)]{.alert}: How much $Y$ changes for a one-unit change in $Z$. Estimated from Eq. (2) (not shown in the figure)
  - [Direct Effect (d)]{.alert}: How much $Y$ changes for a one-unit change in $Z$, *holding $M$ constant*. Estimated from Eq. (3)
  - [Indirect Effect (ab)]{.alert}: How much of $Z$'s effect on $Y$ is transmitted *through* $M$. Calculated as $\hat{a}\hat{b}$ (or $\hat{c} - \hat{d}$)
...
:::{style="margin-top: 30px; font-size: 31px;"}
- Remember from our class on heterogeneous effects: [causal effects often vary because people are different!]{.alert}
...
- Two types of people in our population:
  - **Person A:** [High-benefit type]{.alert}. Training massively boosts their skills ($a_i$ = large), and their new skills make them very employable ($b_i$ = large)
  - **Person B:** [Low-benefit type]{.alert}. Training barely helps their skills ($a_i$ = small), and even with new skills, they are not very employable ($b_i$ = small)
  
- What researchers calculate: [$\hat{a} \times \hat{b} = E[a_i] \times E[b_i]$]{.alert}
  - This is the [product of averages]{.alert} - it treats everyone as if they are "average"
:::
...
- The difference matters because of $Cov(a_i, b_i)$:
  ...
  - [The simple multiplication $\hat{a}\hat{b}$ underestimates the true effect!]{.alert}
  - The covariance term is [generally not zero]{.alert} AND [we cannot estimate it from data]{.alert}, so we can't fix the simple formula
:::
:::
:::
...
# A potential outcomes view {background-color="#1B3A6B"}
...
- This allows us to think about hypothetical scenarios:
  ...
  - These are called [complex potential outcomes]{.alert} because they involve situations that can never happen in reality (more on that soon!)
...
- [Total Effect (ATE):]{.alert} $E[Y_i(1) - Y_i(0)] = E[Y_i(M_i(1), 1) - Y_i(M_i(0), 0)]$
- [Controlled Direct Effect (CDE):]{.alert} Effect of $Z$ on $Y$, holding $M$ fixed at some level $m$
  - $CDE(m) = E[Y_i(m, 1) - Y_i(m, 0)]$
- [Natural Direct Effect (NDE):]{.alert} Effect of $Z$ on $Y$ if $M$ were held at the level it *would have taken* under control
  - $NDE = E[Y_i(M_i(0), 1) - Y_i(M_i(0), 0)]$
- [Natural Indirect Effect (NIE):]{.alert} Effect of $M$ changing from $M_i(0)$ to $M_i(1)$, holding $Z$ fixed (usually at $Z=1$ for policy relevance)
...
- Again, [this can never happen in reality!]{.alert} If $Z_i=1$, we always observe $M_i(1)$, not $M_i(0)$
...
# Can we address this? 🤔 {background-color="#1B3A6B"}
...
- Sharp null hypothesis: $M_i(1) = M_i(0)$ for all individuals $i$
  - That is, [training has no effect on anyone's]{.alert} implicit bias scores
...
- This suggests no evidence that training changes implicit bias on average, [so $M$ is unlikely to mediate the effect of $Z$ on $Y$]{.alert}
- [We don't have unrefutable proof]{.alert}, but some evidence against this mediation pathway
- It is good practice to test [multiple mediators]{.alert} this way to narrow down plausible mechanisms
:::
...
- We can also [manipulate the mediator $M$ directly]{.alert} in an experiment, alongside $Z$
...
- [Controlled Direct Effect]{.alert}: Effect of training holding skills constant = (Group 3 - Group 1)
- [Effect of skills holding training constant]{.alert} = (Group 4 - Group 3)
- [Total training effect]{.alert} = (Group 3 - Group 1)
- [Interaction effect]{.alert} = (Group 4 - Group 3) - (Group 2 - Group 1)
:::

:::{.column width=50%}
- We're [artificially setting]{.alert} skills levels through workshops
- But in reality, skills would develop [naturally]{.alert} from job training
- The [mechanism differs]{.alert} when we force $M$ vs. letting $M$ emerge naturally
- So we get Controlled Direct Effects, [not Natural Direct Effects]{.alert}
:::
:::
:::
...
# A different approach: implicit mediation 💡 {background-color="#1B3A6B"}
...
- [Implicit Mediation Design:]{.alert} (e.g., Baird et al. 2009)
...
| 5. Neighbours       | Encouragement + Monitoring + All Records | 37.8%   |     [+8.1%]{.alert}    |
...
- Very useful for [programme evaluation and refinement!]{.alert}
:::
...
- Using $Z$ as an [instrument for $M$ requires the strong exclusion restriction]{.alert} (no direct $Z \to Y$ path), often violated
