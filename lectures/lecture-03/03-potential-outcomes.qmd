...
- People will choose to participate in a programme when they expect the program to make them better off (i.e., when $Y_{1,i} - Y_{0,i} > 0$).
...
- [Extremely strong]{.alert} (read: often *completely unreasonable* 😅) assumptions are required to make either of these impact evaluation approaches credible
:::
...
- Expected outcomes
- In the absence of the program, the [expected outcome]{.alert} would be the same for both groups.
