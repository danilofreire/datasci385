...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[Interference](https://danilofreire.github.io/datasci385/lectures/lecture-16/16-interference.html)"
transition: slide
...
# Hello! How are you doing? 😊 {background-color="#1B3A6B"}
...
# Interference {background-color="#1B3A6B"}
...
- The [stable]{.alert} part means that [potential outcomes for any unit are unaffected by the treatment status of other units]{.alert}
...
Source: [Aronow et al. (2021)](https://arxiv.org/pdf/2001.05444)
:::
:::
:::
:::
...
- You find a [positive effect]{.alert} of the programme on test scores
...
# How to deal with interference? (good and bad!) {background-color="#1B3A6B"}
...
:::{style="margin-top: 20px; font-size: 26px;"}
- From the [four potential outcomes]{.alert}, some interesting estimands emerge:
...
# Spatial spillovers {background-color="#1B3A6B"}
...
:::{style="margin-top: 20px; font-size: 26px;"}
- Second, the [probability of assignment to each treatment condition varies from one observation to the next]{.alert}
- For instance, village A has a 0.20 probability of being exposed to spillovers from B, whereas village B has a 0.40 probability because of A and C
- As we have seen previously, when the treatment assignment probabilities vary across units, we should [weight observations by inverse probability of assigned experimental condition]{.alert}, excluding subjects with zero probability (e.g., F)
...
| 4 | $Y_{00}$ | $Y_{00}$ | $Y_{10}$ | $Y_{01}$ | $Y_{00}$ | 0.6 | 0.2 | 0.2 |
...
# Within-subject designs and repeated-measures experiments {background-color="#1B3A6B"}
...
-  The pre-post design [can also be a between-subjects design]{.alert} if [some respondents are never exposed to the treatment]{.alert}; respondents difference scores are compared between groups
:::
...
- This is true in some cases: [the no-persistence assumption is likely violated]{.alert} when the treatment gives information or training, for example
- The [quasi-pre-post design]{.alert} is a variation of the pre-post design that tries to avoids this problem by providing a [similar, but not identical, treatment]{.alert} in the pretest phase
  - For instance, a training session on a related but different topic
...
# Waitlist designs {background-color="#1B3A6B"}
...
&- \frac{\frac{7 + 1 + 4 + 3 + 2 + 3}{0.75} + \frac{5 + 3 + 3 + 3}{0.50} + \frac{3 + 4}{0.25}}{\frac{6}{0.75} + \frac{4}{0.50} + \frac{2}{0.25}} = 2.72.
\end{aligned}
$$
:::
