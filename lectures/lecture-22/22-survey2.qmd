...
:::{.columns}
:::{.column width="50%"}
- The estimates have two panels: sensitive item (top) and control items (bottom)

- The ICT regression estimates:
  - **Sensitive item model** - How covariates affect agreement with the sensitive item
  - **Control items model** - How covariates affect responses to non-sensitive control items

- The top panel shows how each variable affects the [log-odds of agreeing with the sensitive item]{.alert}

- **south (1.68)**: Southerners have significantly higher odds of agreeing with the sensitive item (exp(1.68) = 5.4 times higher)
- **age (0.64)**: Each additional decade increases the odds by exp(0.64) = 1.9 times
- **male (0.85)**: Men have exp(0.85) = 2.3 times higher odds than women
- **college (-0.32)**: College-educated individuals have slightly lower odds (exp(-0.32) = 0.73)
:::

:::{.column width="50%"}
- The bottom panel shows how each variable affects the [average number of control items agreed with]{.alert}


- **Intercept (1.19138)**: - Baseline log-odds of endorsing control items
- **south (-0.29202)**: Being from the south decreases the log-odds of endorsing control items
- **age (0.03323)**: Each unit increase in age slightly increases the log-odds
- **male (-0.25059)**: Being male decreases the log-odds
- **college (-0.51641)**: College education decreases the log-odds

- The negative intercept (-5.51) for the sensitive item indicates low baseline prevalence of the attitude being measured
:::
:::
:::
